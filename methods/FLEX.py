...
from methods.PrototypeMan import PrototypeManager
import shutil
...
class FLEX(BaseLearner):
    def __init__(self, args):
        ...
        self.proto_manager = PrototypeManager(self.args)
    ...
    def incremental_train(self, data_manager):
        ...
        if self.args['type'] != -1 and self.args['type'] - 1 > self._cur_task:
            ...
            prototype_loader = DataLoader(
                     train_dataset, batch_size=256, shuffle=False, num_workers=self.args["num_worker"], multiprocessing_context=self.args["mulc"], persistent_workers=True)
            self.proto_manager.generate_prototypes(self._network, prototype_loader)
    ...
    def _fl_train(self, train_dataset, test_loader):
        ...
        for _, com in enumerate(prog_bar):
            ...
            for idx in idxs_users:
                ...
                #     train_dataset, batch_size=256, shuffle=False, num_workers=self.args["num_worker"], multiprocessing_context=self.args["mulc"], persistent_workers=True)
                # self.proto_manager.generate_prototypes(self._network, prototype_loader)
                # current_features, current_labels = self.proto_manager.extract_features(copy.deepcopy(self._network), local_train_loader)
                # self.proto_manager.generate_pseudo_features(current_features, current_labels, self._cur_task, idx)
                ...
                if self._cur_task == 0:
                    ...
                    w, total_syn, total_local, total_loss = self._local_finetune(self._old_network,
                                                                                 ...
                                                                                 idx, scheduler.get_last_lr()[0], com)
    ...
    def _local_finetune(self, teacher, model, train_data_loader, task_id, client_id, lr, com):
        comguide = 80
        ...
        if com >=comguide:
            ...
            total_local = 0.0
            total_syn = 0.0
            #model.convnet의 모든 파라미터를 freeze
            for name, param in model.named_parameters():
                if not ("mapping" in name or "fc" in name):
        ...
        for it in range(self.args["local_ep"]):
            ...
            for batch_idx, (_, images, labels) in iter_loader:
                ...
                #images, labels 로 prototype 생성, extract feature
                if com >=comguide:
                    with torch.no_grad():
                        current_features, current_labels = self.proto_manager.generate_prototypes2(model, images, labels)
                    #pseudo feature 생성 및 불러오기
                
                model.train()
                

                if com >=comguide:
                    current_features, current_labels ,pseudo_features, pseudo_labels= self.proto_manager.generate_pseudo_features2(current_features, current_labels, task_id, client_id)
                    current_features = torch.tensor(current_features).cuda()
                    current_labels = torch.tensor(current_labels).cuda()
                    pseudo_features = torch.tensor(pseudo_features).cuda()
                ...
                if com >=comguide:
                    pseudo=model.fc(combined_features)['logits']
                    pseudo_ce=F.cross_entropy(pseudo, combined_labels)
                ...
                s_loss_f = torch.nn.functional.mse_loss(s_f, t_f.detach())
                loss = cur * (loss_ce_cur + self.ltc * c_loss_f) + pre * (loss_kd + s_loss_f) + pseudo_ce
